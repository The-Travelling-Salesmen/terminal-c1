...
class BasicTests(unittest.TestCase):
    ...
    def test_simple_fields(self):
        ...
        self.assertEqual(
            5, game.get_resource(game.BITS, 1), "My opponent should have 5 bits"
        )
        self.assertEqual(
            25, game.get_resource(game.CORES, 1), "My opponent should have 25 cores"
        )
        self.assertEqual(
            0,
            game.turn_number,
    ...
    def test_get_units_in_range(self):
        ...
        self.assertEqual(
            37,
            len(game.game_map.get_locations_in_range([13, 13], 3)),
    ...
    def _test_get_attackers(self):
        ...
        self.assertEqual(
            [],
            game.get_attackers([13, 13], 0),
            "Are we being attacked by an encryptor?",
        )
        game.game_map.add_unit("FF", [14, 12], 1)
        self.assertEqual(
            [], game.get_attackers([13, 13], 0), "Are we being attacked by a filter?"
        )
    ...
    def future_turn_testing_function(self, game, expected, turns):
        ...
        self.assertAlmostEqual(
            ...
            expected,
            0,
            "Expected {} bits {} turns from now, got {}".format(
                expected, turns, actual
            ),
        )
