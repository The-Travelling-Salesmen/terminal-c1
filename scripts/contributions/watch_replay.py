...
try:
    ...
    if usr_in.lower() == "y" or usr_in.lower() == "yes":
        ...
        try:
            ...
            sys.stderr.write("\n\n{}\n\n".format(str(e)))
            sys.stderr.write(
                "Failed: Check to make sure you have all core dependencies installed (usually TkAgg).\n\n"
            )
            sys.exit()
...
SPEED = {
    ...
    "5": 4,
    "6": 8,
}  # speed versions, key is user input (number)
...
class Unit:
    def __init__(self, t, x, y, hp, p, ID, count, ax):
        ...
        self.patches = (
            []
        )  # these are what get drawn to the animation (contain polygons themselves)
    ...
    def create_patches(self, count, ax):
        if self.unit_type == FILTER:
            wedge = Wedge((self.x, self.y), 0.2, rotate(0), rotate(360), width=0.07)
            ...
            inner_wedge = Wedge(
                (self.x, self.y), 0.12, rotate(0), rotate(360), width=0.03
            )
            outer_wedge = Wedge(
                (self.x, self.y), 0.37, rotate(0), rotate(360), width=0.15
            )
            ...
            inner_wedge = Wedge(
                (self.x, self.y), 0.2, rotate(0), rotate(360), width=0.07
            )
            outer_wedge = Wedge(
                (self.x, self.y), 0.45, rotate(0), rotate(360), width=0.01
            )
            ...
            self.unit_type == PING
            or self.unit_type == EMP
            or self.unit_type == SCRAMBLER
        ):
    ...
    def hp_to_r(self, stability):
        return min((stability - MAX_HP[self.unit_type]) / 50, 0.5)
    ...
    def set_pos(self, x, y):
        ...
        self.y = y
        for patch in self.patches:
            if type(patch) == Polygon:
                patch.set_xy([(x, y) for (x, y) in GET_VERTS[self.unit_type](x, y)])
            elif type(patch) == Circle:
                patch.center = x, y
    ...
    def set_color(self):
        for patch in self.patches:
            ...
            if self.unit_type == EMP or self.unit_type == SCRAMBLER:
    ...
    def check_stability(self, count, ax):
        if self.stability > MAX_HP[self.unit_type]:
            if len(self.patches) == 1:
                extra_wedge = Circle(
                    (self.x, self.y), self.hp_to_r(self.stability), linewidth=4
                )
    ...
    def update(self, x, y, stability, p_index, ID, count, ax):
        ...
        if (
            self.unit_type == FILTER
            or self.unit_type == ENCRYPTOR
            or self.unit_type == DESTRUCTOR
        ):
            self.set_wedge_shape()
...
class PatchWrapper:
    def __init__(self):
        self.units = (
            {}
        )  # stores every unit currently on the board with each ID as the key
        self.loc = (
            {}
        )  # stores the number of units at a location with each location tuple (x,y) as the key
    ...
    def update_units(self, units, ax):
        ...
        for unit_raw in units:
            ...
            try:
                self.units[ID].update(
                    x, y, stability, p_index, ID, self.loc[(x, y)], ax
                )
            except KeyError:
                self.create_unit(
                    unit_type, (x, y), stability, p_index, ID, self.loc[(x, y)], ax
                )

    # adds the count lable to a position on the board
    def plot_text(self, txt, pos, ax):
        x, y = pos
        self.lbls.append(ax.text(x + 0.4, y - 0.4, str(txt), fontsize=10))
...
class Info:
    def __init__(self, endStats, ax, slider_exists=False):
        ...
        self.color = {
            ...
            1: "C0",
            2: "r",
        }  # color reference based on player index
        ...
        if endStats != None:
            ...
            self.winner_name = (
                endStats["player1"]["name"]
                if self.winner == 1
                else endStats["player2"]["name"]
            )
    ...
    def disp_reference(self, slider_exists):
        ...
        if not BLIT and slider_exists:
            self.ax.text(
                0.5,
                0.03,
                "You can also scrub the turn slider with your mouse",
                fontsize=12,
                verticalalignment="bottom",
                horizontalalignment="center",
            )
        ...
        try:
            self.lbls.append(
                self.ax.text(
                    0.5,
                    0.67,
                    "{} wins!".format(self.winner_name),
                    color=self.color[self.winner],
                    verticalalignment="bottom",
                    horizontalalignment="center",
                    fontsize=24,
                )
...
class Plot:
    def __init__(self, data, ax, frame=0):
        ...
        self.data = (
            data
        )  # all known health data, tuple containing two lists, player1 and player2 healths
        ...
        line, = self.ax.plot(
            self.data[0][:0], color="C0"
        )  # create and get a line reference from the plot
        self.lines.append(line)
        line, = self.ax.plot(
            self.data[1][:0], color="r"
        )  # create and get a line reference from the plot
        self.lines.append(line)
        self.ax.set_yticks(list(range(0, 36, 5)))  # set the y_labels and scale

        self.lines[0].set_xdata(
            list(range(0, 100))
        )  # set the x_range for the line to always be 100
    ...
    def update(self, frame, data=None):
        ...
        nulls = [
            None for x in range(100 - frame)
        ]  # extra values for beginning, size must match
        ...
        self.ax.set_xticklabels(
            list(range(x_0, frames + 1, 20))
        )  # updates the x_labels
...
class Graph:
    def __init__(
        ...
        self.fig, ax = plt.subplots(
            nrows=1, ncols=2
        )  # splits the plot into two halves and gets the references
        self.board_ax, self.info_ax = ax  # assign left and right side references
        self.plot_ax = self.fig.add_subplot(
            324
        )  # add the plot and assing it's reference

        self.general_init(
            data, frames_in_turn, healths
        )  # handles general initialization (called again if in real-time)

        self.head = (0, -1)  # tracks the current turn, frame pair
        self.end_of_game = False  # end of game flag
        self.is_manual = (
            False
        )  # stores whether the user is manually moving the slider, keyboard, etc
        self.single_advance = (
            False
        )  # true when user is scrubbing, but still want to move forward one frame
        self.stop_slider_evt = (
            False
        )  # stop the slider event from triggereing when the code changes it

        self.patches = PatchWrapper()  # creates the PatchWrapper object

        self.stream = (
            self.data_stream()
        )  # gets a data_reference - this passes all data to the animation

        self.setup_board()  # initialize static parts of the board

        self.fig.canvas.mpl_connect(
            "key_press_event", self.keyboard_input
        )  # connect keyboard events to the keyboard_input function
    ...
    def save_animation(self, save_name, writers, keep_trying):
        ...
        try:
            name, given_ext = save_name.split(".")
        except ValueError:
            name, given_ext = save_name, ""

        default = [
            "ffmpeg",
            "pillow",
            "html",
        ]  # this is the default order of priority for running a save

        # when this block finishes, attempts is a list of possible attempts in order of priority
        attempts = (
            [ex_to_writer[given_ext]]
            if given_ext != "" and given_ext in ex_to_writer.keys()
            else default
        )
        attempts = attempts if "empty" in writers else writers
        attempts = (
            attempts + [w for w in default if w not in attempts]
            if keep_trying
            else attempts
        )
        ...
        for i, writer in enumerate(attempts):
            try:
                ...
                print(
                    "{} is not a valid writer. Options are:\n\t- ffmpeg  (for .mp4 videos)\n\t- pillow  (for gifs)\n\t- html    (for browser view and individual frames)".format(
                        writer
                    )
                )
    ...
    def get_extension(self, writer, given_ext):
        ...
        if given_ext != extensions[writer] and given_ext != "":
            print(
                "You used extension .{0}, but .{2} is the valid type for {1}. Using .{2}:".format(
                    given_ext, writer, extensions[writer]
                )
            )
    ...
    def check_ffmpeg(self):
        ffmpeg_process = subprocess.Popen(
            "ffmpeg", shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )
    ...
    def general_init(self, data, frames_in_turn, healths):
        self.data = (
            data
        )  # dict with keys of (turn, frame) tuple and values of a Frame object
        self.frames_in_turn = (
            frames_in_turn
        )  # dict with keys of turn and values of number of frames in that turn
        self.healths = (
            healths
        )  # all known health data, tuple containing two lists, player1 and player2 healths
        ...
        self.plot = Plot(
            self.healths, self.plot_ax
        )  # create the Plot object (plots the health)

        # try and get endStats, if not then file is still being created by engine (game is still running)
        try:
            last_frame = max(
                self.data, key=lambda f: (f[0], f[1])
            )  # the last frame of the entire match (single number)
            endStats = self.data[last_frame].data[
                "endStats"
            ]  # here is where the error would be thrown - if endStats exists
            ...
            if not BLIT:
                self.slider = Slider(
                    self.fig.add_axes([0.6, 0.03, 0.3, 0.03]),
                    "Turn Slider",
                    0,
                    self.num_frames,
                    valfmt="%1i",
            ...
            self.info = Info(
                endStats, self.info_ax, True
            )  # create the Info (right side) with endStates information
            self.real_time = False  # not longer running in real-time
        except KeyError as e:
            self.info = Info(
                None, self.info_ax
            )  # endStats doesn't exist, create Info with default values

    # change the interval speed between frames
    def change_play_speed(self, speed):
        self.speed = speed
        self.anim.event_source.interval = 100.0 / SPEED[self.speed]
    ...
    def slider_active(self, val):
        if (
            self.stop_slider_evt
        ):  # don't update again if value was changed by code and not user
            return
        self.head = self.val_to_frame_turn(
            val
        )  # update the head based on the slider value
    ...
    def update_slider(self, pair):
        ...
        if self.end_of_game:
            self.slider.set_val(int(self.frame_turn_to_val(turns, frames)) + 2)
        else:
            self.slider.set_val(int(self.frame_turn_to_val(turns, frames)))
    ...
    def data_stream(self):
        while True:
            ...
            if self.real_time:
                ...
                try:
                    self.frame_generator.send(
                        self.num_frames
                    )  # send the inverval generator the number of frames loaded
                except TypeError:
                    pass
            ...
            units = self.cache_units(p1Units, 1) + self.cache_units(
                p2Units, 2
            )  # format the unit data into how it is passed to my functions
            ...
            self.plot.update(
                self.frame_turn_to_val(self.head[0], self.head[1])
            )  # update the health plot
    ...
    def cache_units(self, units, p_index):
        ...
        units_new = []
        for unit in filters:
            units_new.append((FILTER, (unit[0], unit[1]), unit[2], p_index, unit[3]))
        for unit in encryptors:
            units_new.append((ENCRYPTOR, (unit[0], unit[1]), unit[2], p_index, unit[3]))
        for unit in destructors:
            units_new.append(
                (DESTRUCTOR, (unit[0], unit[1]), unit[2], p_index, unit[3])
            )
        for unit in pings:
            units_new.append((PING, (unit[0], unit[1]), unit[2], p_index, unit[3]))
        for unit in emps:
            units_new.append((EMP, (unit[0], unit[1]), unit[2], p_index, unit[3]))
        for unit in scramblers:
            units_new.append((SCRAMBLER, (unit[0], unit[1]), unit[2], p_index, unit[3]))
    ...
    def check_end_of_game(self):
        ...
        if self.end_of_game:
            self.info.show_winner()  # show the winner if it is the end of game
...
class Replay:
    def __init__(self, f_name):
        ...
        self.frames = (
            {}
        )  # dict containing all data, keys are turn, frame tuple with Frame objects as values
            ...
            for line in f:
                ...
                if line != "":
                    ...
                    try:
                        ...
                        self.frames[(turn_num, frame_num)] = Frame(
                            turn_num, frame_num, data
                        )
...
def run_match(a1="", a2=""):
    ...
    default_algo = (
        parent_dir + "/algos/starter-algo-ZIPME/run.ps1"
        if is_windows
        else parent_dir + "/algos/starter-algo-ZIPME/run.sh"
    )
    algo1 = default_algo
    algo2 = default_algo
...
def main(args):
    ...
    if args["run_match"][0] != "empty":
        ...
        previous_replay = str(
            fh.get_last_replay()
        )  # get the replay that was last created

        if len(args["run_match"]) > 1:
            run_match(
                args["run_match"][0], args["run_match"][1]
            )  # run the match with both algos specified
        else:
            run_match(args["run_match"][0])  # run the match with one algo specified
        ...
        while True:
            try:
                ...
                animatedReplay = Graph(
                    replay.frames,
                    replay.frames_in_turn,
                    replay.healths,
                    writers,
                    keep_trying,
                    fh=fh,
        ...
        if save == "" and "empty" not in writers:
            print(
                "You specified a writer type, but it will not be used since you are not saving the animation"
            )
        elif save == "" and keep_trying:
            print(
                "You specified to keep trying writers, but it will not be used since you are not saving the animation"
            )
