    ...
    try:
        ...
        if usr_in.lower() == "y" or usr_in.lower() == "yes":
            ...
            try:
                ...
                sys.stderr.write("\n\n{}\n\n".format(str(e)))
                sys.stderr.write(
                    "Failed: Check to make sure you have all dependencies installed.\n\n"
                )
                sys.exit()
    except:
        plt_installed = False
...
class Algo:
    ...
    def add_data(self, replay, turn, arg, data, cumulative=False):
        ...
        if cumulative:
            try:
                self.replays[replay][turn][arg] = (
                    self.replays[replay][turn - 1][arg] + data
                )
            except KeyError:
    ...
    def print_block(self, header, data):
        ...
        for arg in data:
            val = (
                round(data[arg], 1)
                if type(data[arg]) == int or type(data[arg]) == float
                else data[arg]
            )
            sys.stderr.write(
...
class Replay:
    ...
    def add_data_to_algo(self, algo, t, f, stats, units, spawn):
        ...
        algo.add_data(
            ...
            t,
            "cores_on_board",
            self.get_cores_on_board(filters, encryptors, destructors),
        )

        if f == 0:
            algo.add_data(
                self.fname, t, "cores_spent", self.get_cores_spent(algo, spawn), True
            )
            algo.add_data(
                self.fname, t, "bits_spent", self.get_bits_spent(algo, spawn), True
            )
    ...
    def unpack_data(self, algos):
        try:
            ...
            self.algo1.add_end_stats(
                self.fname, self.turns[self.valid_turns[-1]]["endStats"]["player1"]
            )
            self.algo2.add_end_stats(
                self.fname, self.turns[self.valid_turns[-1]]["endStats"]["player2"]
            )
...
class FileHandler:
    ...
    def add_plot(self, lbl):
        if lbl == "wins":
            ...
            Graph.add_bar(
                y_pos, wins, lbls, y_ticks, "# of Wins", "Number of Wins per Algo"
            )
        elif lbl == ":":
            Graph.advance()
...
def main(args):
    ...
    fh = FileHandler()
    fh.load_files(
        int(args["num"]), args["all"], args["file"]
    )  # loads the files - all JSON reading is here

    # check to see if matplotlib is installed
    graphing_enabled = (
        True if len(verbose_options) > 0 or len(summary_options) > 0 else False
    )
    ...
    if args["all"]:
        run_every_replay_verbose(fh, graphing_enabled, options) if args[
            "verbose"
        ] else ""
