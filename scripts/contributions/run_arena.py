...
def run_single_game(process_command, algo1, algo2, max_name_len):
    p = subprocess.Popen(
        process_command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE
    )
    ...
    print(
        "{: <30}{: <{fill}}   vs   {}".format(
            "Finished running match:", algo1, algo2, fill=str(max_name_len)
        )
    )
...
def run_match(arg1="", arg2="", max_name_len=0):
    ...
    run_single_game(
        ...
        algo1.split("\\")[-2].replace("algos/", ""),
        algo2.split("\\")[-2].replace("algos/", ""),
        max_name_len,
    )
...
def run_matches(matches, batch_size):
    ...
    for i in range(len(processes)):
        print(
            "{: <30}{: <{fill}}   vs   {}".format(
                "Starting match:", algos[i][0], algos[i][1], fill=str(max_name_len)
            )
        )
        processes[i].start()
        i += 1
